...
- 既存コマンドの全面置き換え（互換性を壊す変更）
  - 例外: review-loop の設定SoT統一を目的としたコマンド名称刷新（`/codex-pr-review` -> `/pr-bots-review`）は、Issue #108 の範囲で許容する
...
測定方法: `scripts/agentic-sdd/lint-sot.py`（仮）をローカルで実行し、違反ケースで exit code と出力を確認
...
- 2026-02-21: v1.1 Q5「作らない範囲」に Issue #108 の例外条項を追加（@ToaruPen）
