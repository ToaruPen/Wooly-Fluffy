...
- 職員（STAFF画面で緊急停止/復帰、セッションリセット、セッション要約候補のConfirm/Deny）

### Q3: 何ができるようになる？

- モード表示なし（`ROOM`/`PERSONAL` 等の表示や切り替え操作を子ども側に出さない）
- 会話はroomセッションとして扱い、最後の会話からアイドル5分でセッション終了できる
- Push-to-talk（PTT）はKIOSK（デフォルトON）で操作でき、待機中は無反応にできる
...
- KIOSKがPTTを開始/終了でき、録音開始/停止し、STT結果に応じて会話が進む
...
```
As a 職員,
I want to 緊急停止/復帰とセッションリセットを操作できる,
So that 誤動作時に安全に復旧し、運用上の事故を減らせる.
```
...
FR-2
機能名: Push-to-talk（KIOSK操作）
説明: 待機中は無反応。KIOSK（デフォルトON）が hold-to-talk（押下中のみ収録）でPTT操作できる。STAFFはPTTを持たず、緊急停止/復帰とセッションリセットを操作できる。緊急停止は入力/出力を中断できる。
優先度: Must
