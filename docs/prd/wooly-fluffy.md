...
- [ ] AC-1: 会話が行われた後、アイドル5分でセッションが終了し、要約（短いタイトル + 構造化JSON）が `pending` として作成される
- [ ] AC-2: `pending` は7日で自動失効（削除）する
- [ ] AC-3: 職員が `pending` をConfirmした場合、対象が `confirmed` になり、`confirmed` は期限なしで保持される
- [ ] AC-4: 職員が `pending` をDenyした場合、対象は削除される（本文保持しない）
- [ ] AC-5: AI応答は文単位で段階的に音声出力され、セグメント順序が維持される（逆順/重複なし）

### 異常系（必須: 最低1つ）

- [ ] AC-E1: セッション要約の生成が失敗/タイムアウトした場合でも、会話は継続しシステムは停止しない（安全なフォールバックに落ちる）
...
- [ ] AC-E5: stop_output指示後、in-flightの旧セグメントが再生されない（stale混入を防止）
