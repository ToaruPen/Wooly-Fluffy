...
from: STAFF UI
to: API Server
用途: PTT開始/終了、強制ROOM、緊急停止/復帰などのイベント入力
プロトコル: HTTP
...
メソッド: POST
説明: PTT/強制ROOM/緊急停止/復帰などのSTAFFイベント入力
...
シナリオ-4（キャンセル: 強制ROOM/緊急停止）

- 事象: STT/会話生成/ツール実行中に `STAFF_FORCE_ROOM` または `STAFF_EMERGENCY_STOP` が入る
...
概要: `ROOM/PERSONAL(name)`、無操作300秒、同意30秒、フォールバック、優先度（強制ROOM/緊急停止）を純粋関数として固定する
推定行数: 200-300行
依存: -
...
影響度: 高
対策: request_id相関、重複排除ID、タイムアウト、強制ROOM/緊急停止の優先順位を仕様として固定し、テストで担保
