...
> **目的**: 実装判断がブレやすい「異常系」「リソース解放」「イベント順序/競合」を先に固定する。
> Issue作成時は、ここで列挙したものを各IssueのAC/テスト観点に落とす。

対象境界:
- HTTP: [例: /api/...]
- Realtime: [SSE/WS の message type と payload]
- Browser API: [録音/WebAudio/3D]
- subprocess/file/DB: [一時ファイル/接続/プロセス]

チェック項目（必要なものだけ埋める）:
- 入力不正: [無効payload/不足フィールド/型不一致] のときの期待（安全なエラー/フォールバック）
- タイムアウト/キャンセル: [期限/中断条件/中断時の挙動]（ハングしない）
- リソース解放: [track/connection/process/tmp file] を **成功/失敗/中断/アンマウント** で確実に解放
- レース条件: [start/stop連打、重複/順序逆転、再接続] の期待（どちらを優先/無視するか）
- データ最小化: [ログ/永続化] に出さない（必要なメタ情報のみ）
