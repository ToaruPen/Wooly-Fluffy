...
> **注意**: lint/format/typecheck は必須。加えて「型チェックを厳格モードで実行する方針」と「テストカバレッジ計測方針（しきい値含む）」を明記すること。
> 対象言語で「高速」「現行メンテ」「評判/採用実績」を満たすツールを選ぶ。
...
選択: [例: mypy / tsc]
選定理由: [理由]
厳格設定: [例: mypy --strict / tsc --strict]
ローカル実行方法: [方法]
CI実行方法: [方法]

ツールチェーン-4
カテゴリ: coverage
選択: [例: pytest-cov / vitest coverage / jest --coverage]
選定理由: [理由]
しきい値方針: [例: line >= 80% / branch >= 70%]
ローカル実行方法: [方法]
CI実行方法: [方法]
