...
このEpicは「見た目・レイアウト・インタラクション・文言」の改善に限定し、PRDの主要フロー（PTT、同意、pending確認、アクセス制御、SSE契約）とデータ最小化方針は変更しない。
...
- STAFF UI: ログイン画面、ステータス表示、PTTボタン、緊急停止、pendingカードの見た目/操作性を改善
- レスポンシブ改善（複数ブレークポイント）
...
- 代表端末プロファイル（固定）
  ...
  - STAFF: スマホ幅〜デスクトップ。特にスマホでもPTT/緊急停止が成立することを優先
- エラー文言はユーザー別に分ける
...
- レスポンシブの優先度は STAFF を最優先（スマホでもPTT/操作が成立する）。KIOSKはフルHD固定表示を主としつつ崩れない範囲を担保する
...
- STAFF: PTT ボタンの `aria-pressed` は維持する
...
Issue-3
番号: 3
Issue名: STAFF リデザイン（ログイン/ステータス/PTT/緊急停止/pending）
...
概要: /kiosk と /staff の主要要素（見出し/モーダル/ログイン/PTT）を安定して検証できるよう調整する。原則として reduced-motion を強制し、アニメーション起因のフレークを抑える
推定行数: 80-150行
依存: #2, #3, #5
