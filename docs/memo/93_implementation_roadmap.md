...
- 実装対象（`.specs/04_data_policy_and_memory_model.md`）
  - `pending`（24h TTL）/ `confirmed` / `ROOM`「みんなの思い出」
...
### M4: KIOSK UI（PTT/フォールバック）+ STAFF UI（運用最低ライン）

狙い: 現場で使える“最低限の形”にする（見守り前提で破綻しない）。

- KIOSK（必須）
  - PTT（物理ボタン=keydown/keyup想定、画面ボタンも用意）
  - 「はい/いいえ」ボタン（STT誤認対策）
...
- 実装/整備対象
  ...
  - 暗号化バックアップ（`confirmed` + `ROOM`のみ、片方向スナップショット）
