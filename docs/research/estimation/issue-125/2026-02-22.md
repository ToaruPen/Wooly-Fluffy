...
- Q6-5（PII/機密）: Yes（PRDでデータ最小化制約が明示され、依存導入時も会話本文/音声/STT全文を扱わない前提確認が必要）
- Q6-6（監査ログ）: No（PRD上の監査ログ要件は No）
- Q6-7（性能）: Yes（PRDで性能要件が Yes のため、依存導入が検証コマンドや実行時挙動に与える影響確認が必要）
- Q6-8（可用性）: No（PRD上の可用性要件は No）
