...
Issue #103 の受け入れ条件（イベント駆動化、bot allowlist、一貫フォーマット通知、後方互換、fail-fast）を、既存の `scripts/watch-codex-review.sh` と整合した最小変更で実現するために、不確実性（イベントpayload差分、権限不足時挙動、通知フォーマット）を先に潰して見積もり幅を狭める。
...
- 直接の先行事例が2件未満: Yes
  - 補足: このリポジトリにはポーリング監視（`scripts/watch-codex-review.sh`）はあるが、PRレビュー系3イベントを横断して同一フォーマット通知する workflow 実装は未存在。
...
候補-4
概要: `README.md` に「イベント駆動推奨、ポーリングfallback」を明記し既存 `scripts/watch-codex-review.sh` は維持する
適用可否: Yes
...
- 何を調べたか: `gh issue view 103`、`scripts/watch-codex-review.sh`、`README.md`、`.agent/commands/codex-pr-review.md`、GitHub Docs（Actions events / token auth）
