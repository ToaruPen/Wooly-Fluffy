...
適用可否: Yes
仮説: コマンド正本（`.agent/commands`）と生成経路（`scripts/sync-agent-config.sh`）を同時更新すれば、旧名称の再注入を防いで AC1/AC4 を満たせる
...
調査結果: review-loop 側（`scripts/codex-review-event.sh`）も未設定時に非0終了へ寄せる方針が既に採用されており、autofix 側も同ポリシーにそろえるのが一貫的
根拠:
- https://github.com/ToaruPen/Agentic-SDD/issues/108
