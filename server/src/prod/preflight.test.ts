...
describe("runPreflight", () => {
  it("fails when STAFF_PASSCODE is missing", async () => {
  ...
  it("fails when WHISPER_CPP_CLI_PATH is missing", async () => {
  ...
  it("fails when WHISPER_CPP_CLI_PATH is not executable", async () => {
    ...
    const result = await runPreflight({ env: baseEnv, fetch: okFetch, fs_access: failingFsAccess });
  ...
  it("fails when WHISPER_CPP_MODEL_PATH is not readable", async () => {
    const fsConstants = await readFsConstants();
    const failingFsAccess = async (path: string, mode: number): Promise<void> => {
  ...
  it("fails when TTS engine is unreachable", async () => {
  ...
  it("fails when LLM_BASE_URL is missing for local provider", async () => {
  ...
  it("fails when local LLM_BASE_URL is unreachable", async () => {
    const failingFetch: typeof globalThis.fetch = async (input) => {
  ...
  it("fails when LLM_API_KEY is missing for external provider", async () => {
    const env = {
      ...baseEnv,
  ...
  it("collects all errors instead of failing fast", async () => {
    ...
    const result = await runPreflight({ env, fetch: failingFetch, fs_access: failingFsAccess });
  ...
  it("accepts GEMINI_API_KEY for gemini_native and skips LLM_BASE_URL reachability", async () => {
    const env = {
      ...baseEnv,
