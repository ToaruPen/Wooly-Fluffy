...
リスク/検証:
- リスク: メタデータ肥大化で可読性低下
- 検証方法: 既存テスト（`scripts/agentic-sdd/tests/test-review-cycle.sh`）で互換を確認
...
- テスト計画への影響: `scripts/agentic-sdd/tests/test-review-cycle.sh` を中心に metadata 互換・reuse条件・fail-fast条件の回帰テスト追加が必要
